...
export const registerValidation: ValidationChain[] = [
  ...
  body('telegram')
    ...
    .isLength({ min: 5 })
    .withMessage('Telegram должен быть не менее 5 символов'),
  body('password')
    .isLength({ min: 6 })
...
export const createOrderValidation: ValidationChain[] = [
  ...
  body('detailedDescription')
    .notEmpty()
    .withMessage('Требуется подробное описание')
    .isLength({ min: 10 })
    .withMessage('Подробное описание должно быть не менее 10 символов'),
  body('price')
    .isNumeric()
